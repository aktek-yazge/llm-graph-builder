...
# Use --pool=solo to avoid SIGSEGV with MPS (Metal Performance Shaders) on Mac
# Prefork pool causes segmentation faults with PyTorch MPS
uv run python -m celery -A src.celery_app worker --loglevel=info --pool=solo &
WORKER_PID=$!
