...
class GenericGraphExecutor:
    ...
    def create_graph_from_llm_output(
        ...
        try:
            ...
            logging.info(f"📊 GenericGraphExecutor: {len(nodes)} nodes, {len(relationships)} relationships")
            ...
            for node in nodes:
                try:
                    ...
                    error_msg = f"Node creation error: {node.get('id', 'unknown')} - {e}"
            ...
            try:
                linked_count = self._link_entities_to_document(file_name, created_node_ids)
                result["document_linked"] = linked_count > 0
                logging.info(f"📎 {linked_count} entities linked to Document: {file_name}")
            except Exception as e:
            ...
            if result["errors"]:
                result["success"] = result["nodes_created"] > 0  # Partial success if any nodes created
    ...
    def _create_relationship(
        ...
        if not all([from_id, to_id, rel_type]):
            logging.warning(f"⚠️ Invalid relationship: missing from_id, to_id, or type - {rel}")
            return False
        ...
        try:
            ...
            if result:
                logging.debug(f"✅ Relationship created: ({from_id})-[{rel_type}]->({to_id})")
                return True
            else:
                logging.warning(f"⚠️ Relationship not created (nodes may not exist): ({from_id})-[{rel_type}]->({to_id})")
                return False
