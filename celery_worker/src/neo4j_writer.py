...
def neo4j_write_task(
    self, operation: str, params: Dict[str, Any], database: str = None
):
    """
    ...
    try:
        with driver.session(database=database) as session:
            ...
            logger.info(
                f"✅ Neo4j Write: {operation} completed - {params.get('chunk_id', params.get('file_name', 'unknown'))}"
            )
...
def neo4j_batch_write_task(
    self, operations: List[Dict[str, Any]], database: str = None
):
    """
        ...
        with driver.session(database=database) as session:
            ...
            def tx_function(tx):
                ...
                for op in operations:
                    ...
                    tx_results.append(
                        {"operation": operation, "status": "success", "result": result}
                    )
                return tx_results
...
def enqueue_neo4j_write(
    operation: str, params: Dict[str, Any], database: str = None, priority: int = 5
):
    """
...
def enqueue_neo4j_batch_write(
    operations: List[Dict[str, Any]], database: str = None, priority: int = 5
):
    """
...
def enqueue_create_chunk_relationship(
    file_name: str, chunk_id: str, position: int, database: str = None
):
    """Create HAS_CHUNK relationship between Document and Chunk"""
    return enqueue_neo4j_write(
        "create_chunk_relationship",
        {
            "file_name": file_name,
            "chunk_id": chunk_id,
            "position": position,
...
def enqueue_create_first_chunk_relationship(
    file_name: str, chunk_id: str, database: str = None
):
    """Create FIRST_CHUNK relationship"""
    return enqueue_neo4j_write(
        "create_first_chunk_relationship",
        {
            "file_name": file_name,
            "chunk_id": chunk_id,
        },
...
def enqueue_create_next_chunk_relationship(
    from_chunk_id: str, to_chunk_id: str, database: str = None
):
    """Create NEXT_CHUNK relationship between chunks"""
    return enqueue_neo4j_write(
        "create_next_chunk_relationship",
        {
...
def enqueue_update_chunk_embedding(
    chunk_id: str, embedding: List[float], database: str = None
):
    """Update chunk embedding"""
    return enqueue_neo4j_write(
...
def enqueue_custom_query(
    query: str, params: Dict[str, Any] = None, database: str = None
):
    """Execute a custom Cypher query"""
    return enqueue_neo4j_write(
        "custom_query",
