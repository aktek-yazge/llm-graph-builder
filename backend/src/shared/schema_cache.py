...
_cached_raw_schema: Optional[Dict[str, Any]] = (
    None  # Raw APOC schema (for DSL validation)
)
_version_check_time: Optional[float] = (
    None  # Son version kontrolü zamanı (Unix timestamp)
)
VERSION_CHECK_TTL_SECONDS = (
    60  # Version kontrolü TTL (saniye) - bu süre içinde PostgreSQL'e gidilmez
)
...
class SchemaVersionCache:
    ...
    def get_schema(self, database_url: str, graph) -> str:
        ...
        if (
            ...
            logger.debug(
                f"✅ Schema RAM'den alındı (TTL cache hit, {VERSION_CHECK_TTL_SECONDS - (current_time - _version_check_time):.0f}s kaldı)"
            )
    ...
    def _get_version(self, database_url: str) -> int:
        ...
        try:
            ...
            if (
                "undefined" in error_str
                or "does not exist" in error_str
                or "relation" in error_str
            ):
                logger.warning(
                    f"⚠️ schema_version tablosu bulunamadı, yeniden oluşturuluyor..."
                )
                db.rollback()
                try:
                    Base.metadata.create_all(bind=self.engine)
    ...
    def _fetch_schema_apoc(self, graph) -> str:
        ...
        try:
            ...
            logger.info(
                f"📦 Raw APOC schema global cache'e kaydedildi ({len(schema_data)} node/rel)"
            )
            ...
            # Count'a göre sırala
            sorted_nodes = sorted(
                nodes.items(), key=lambda x: x[1]["count"], reverse=True
            )
            ...
            if policy_subtypes:
                subtypes_str = ", ".join(
                    [
                        f"{name}({cnt})"
                        for name, cnt in sorted(policy_subtypes, key=lambda x: -x[1])
                    ]
                )
                lines.append("")
                lines.append(
                    "# POLICY SUBTYPES (Policy label'ının alt-tipleri, her biri aynı zamanda Policy'dir)"
                )
                lines.append(f"# Tüm poliçeler için: MATCH (p:Policy)")
                lines.append(
                    f"# Belirli tip için: MATCH (p:TrafikPolicy) veya MATCH (p:KaskoPolicy) vb."
                )
            ...
            for from_l, rel, to_l in relationships:
                ...
                normalized_from = (
                    "Policy" if from_l in policy_subtype_labels else from_l
                )
            ...
            schema = "\n".join(lines)
            logger.info(
                f"✅ APOC Schema: {len(schema)} karakter, {len(main_nodes)} node, {len(seen)} pattern ({elapsed:.2f}s)"
            )
            return schema
    ...
    def _format_langchain_schema(self, structured: dict) -> str:
        ...
        for node_type, props in node_props.items():
            prop_str = ",".join(
                [f"{p['property']}:{p.get('type', 'str')[:3]}" for p in props]
            )
    ...
    def _fetch_schema_lightweight(self, graph) -> str:
        ...
        try:
            ...
            try:
                ...
                labels_result = graph.query(
                    "CALL db.labels() YIELD label RETURN label, 0 as cnt"
                )

            labels_with_count = (
                [(r["label"], r["cnt"]) for r in labels_result] if labels_result else []
            )
            ...
            rel_types = (
                [r["type"] for r in rel_types_result] if rel_types_result else []
            )
            ...
            for label, count in main_labels:
                ...
                for prop in props:
                    if prop in type_mapping:
                        ...
                        x in prop.lower()
                        for x in ["id", "name", "text", "content", "title"]
                    ):
                        prop_type = "str"
                    elif any(x in prop.lower() for x in ["date", "time"]):
                        prop_type = "dt"
                    elif any(
                        x in prop.lower() for x in ["count", "number", "amount", "year"]
                    ):
                        prop_type = "int"
                    else:
                        prop_type = "str"
            ...
            for from_label, rel_type, to_label in patterns:
                # Policy alt-tiplerini Policy olarak değiştir
                normalized_from = (
                    "Policy" if from_label in policy_subtype_labels else from_label
                )
                normalized_to = (
                    "Policy" if to_label in policy_subtype_labels else to_label
                )
            ...
            schema = "\n".join(lines)
            logger.info(
                f"✅ Schema oluşturuldu: {len(schema)} karakter, {len(main_labels)} node, {len(seen_patterns)} pattern"
            )
            return schema
        ...
        for node_data in nodes_result or []:
            ...
            for prop_name in properties:
                if prop_name in type_mapping:
                    ...
                    x in prop_name.lower() for x in ["id", "name", "address", "content"]
                ):
                    prop_type = "str"
                else:
                    prop_type = "str"
