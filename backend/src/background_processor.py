...
class BackgroundProcessor:
    ...
    async def process_next_file(self):
        """Process next file in queue (V1: QUEUED files)"""
        try:
            ...
            logging.info(
                f"🔄 Starting processing file: {file_to_process.filename} (ID: {file_to_process.id})"
            )
    ...
    async def reset_stuck_processing_files(self):
        ...
        try:
            ...
            try:
                ...
                if reset_count > 0:
                    db_session.commit()
                    logging.info(f"✅ Reset {reset_count} stuck processing file(s) on startup")
                else:
        ...
        try:
            ...
            try:
                ...
                if files_needing_extraction:
                    ...
                    await self.process_v2_image_extraction_batch(
                        files_needing_extraction
                    )
                ...
                if files_ready_for_chunking:
                    ...
                    await self.process_v2_chunking_batch(files_ready_for_chunking)
                    return  # Return after processing batch
                ...
                if files_ready_for_graph:
                    ...
                    await self.process_v2_graph_creation_batch(files_ready_for_graph)
                    return  # Return after processing batch
            ...
            try:
                ...
                if os.path.exists(images_dir):
                    ...
                    for img_file in os.listdir(images_dir):
                        if img_file.startswith(f"{doc_name}_page_") and img_file.endswith('.png'):
                ...
                if local_images_exist or s3_images_exist:
                    ...
                    if local_images_exist:
                        ...
                        page_images = [os.path.basename(img) for img in local_image_files]
                    else:
                        # Use S3 image names (images exist in S3 but not locally)
                        generated_images = []
                        page_images = s3_image_names
                    
                    logging.info(
                        f"✅ V2: Using {len(page_images)} existing page images for: {file_record.original_name}"
                    )
                else:
    ...
    async def process_v2_chunking_batch(self, files: list):
        ...
        logging.info(f"📖 V2: Starting chunking batch for {len(files)} files (eş zamanlı)")

        # Process all files concurrently using asyncio.gather
        tasks = [self._process_single_file_chunking(file_record) for file_record in files]
        await asyncio.gather(*tasks, return_exceptions=True)
    ...
    async def process_v2_graph_creation(self, file_record: UploadedFile):
        """Process V2 graph creation for a file (after chunking)"""
        try:
            # Check if graph creation is needed
            if file_record.graph_status != "pending":
                logging.info(
            ...
            logging.info(
                f"🎨 V2: Starting graph creation for: {file_record.original_name} (ID: {file_record.id})"
            )
            ...
            try:
                ...
                if file_record:
                    ...
                    file_record.model_used = (
                        file_record.model_used or "openai_gpt_4o_mini"
                    )
                    file_record.generate_embedding = (
                        file_record.generate_embedding or "false"
                    )
            ...
            asyncio.create_task(
                score_module.process_graph_creation_v2(
                    ...
                    password=password,
                    database=database,
                    generate_embedding=file_record.generate_embedding == "true",
                )
            )
    ...
    async def process_v2_graph_creation_batch(self, files: list):
        ...
        logging.info(f"🎨 V2: Starting graph creation batch for {len(files)} files (eş zamanlı)")

        # Process all files concurrently using asyncio.gather
        tasks = [self._process_single_file_graph_creation(file_record) for file_record in files]
        await asyncio.gather(*tasks, return_exceptions=True)
    ...
    async def _process_single_file_graph_creation(self, file_record: UploadedFile):
        ...
        try:
            ...
            try:
                ...
                asyncio.create_task(self.process_v2_graph_creation(file_record))
