...
class QueueManager:
    ...
    def check_file_duplicate(self, file_path: str, filename: str) -> Optional[UploadedFile]:
        """
        ...
        try:
            ...
            if existing_file:
                logging.info(f"🔍 Duplicate file detected: {filename} matches existing {existing_file.filename} (ID: {existing_file.id})")
    ...
    def check_file_in_neo4j(self, filename: str, neo4j_uri: str, 
        ...
        """
        try:
            graph = create_graph_database_connection(neo4j_uri, username, password, database)
            if not graph:
                return False
            ...
            if doc_count > 0:
                logging.info(f"📊 File already exists in Neo4j: {filename} ({doc_count} documents)")
                return True
    ...
    def add_file_with_checks(self, 
                           ...
                           override_duplicate: bool = False) -> Tuple[UploadedFile, bool, str]:
        """
        ...
        try:
            ...
            if self.check_processed_file_exists(filename, neo4j_uri) and not override_duplicate:
    ...
    def get_queue_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "oldest_queued": queued_files[0].created_at.isoformat() if queued_files else None,
                "current_processing": processing_files[0].filename if processing_files else None
            }
    ...
    def cleanup_old_files(self, days_old: int = 30, status_filter: List[FileStatus] = None) -> int:
        """
                ...
                for file_record in files:
                    if file_record.created_at < cutoff_date:
                        ...
                        if self.db.delete_file(file_record.id):
                            cleaned_count += 1
                            logging.info(f"✅ Cleaned up old file record: {file_record.filename}")
    ...
    def validate_queue_integrity(self) -> Dict[str, Any]:
        ...
        try:
            issues = []
            all_files = self.db.get_all_files(limit=1000)  # Get more files for validation
            ...
            for file_record in all_files:
                ...
                if not file_path.exists():
                    issues.append(f"Missing file: {file_record.filename} (ID: {file_record.id})")
                    missing_files += 1
                    continue
                
                # Check file size consistency
                actual_size = file_path.stat().st_size
                if file_record.file_size and abs(actual_size - file_record.file_size) > 100:  # Allow small differences
                    issues.append(f"Size mismatch: {file_record.filename} - Expected: {file_record.file_size}, Actual: {actual_size}")
                    size_mismatches += 1
