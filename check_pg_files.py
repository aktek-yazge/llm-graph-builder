...
with engine.connect() as conn:
    ...
    if rows:
        print("\n📊 Son 15 Dosya İşlemi:\n")
        print(
            f"{'ID':<6} {'Original Name':<50} {'Upload':<12} {'Chunking':<12} {'Graph':<12} {'Embedding':<12} {'Updated At':<20}"
        )
        print("-" * 140)
        ...
        for row in rows:
            ...
            print(
                f"{id_val:<6} {original_name_short:<50} {upload_status:<12} {chunking_status:<12} {graph_status:<12} {embedding_status:<12} {updated_at_str:<20}"
            )
