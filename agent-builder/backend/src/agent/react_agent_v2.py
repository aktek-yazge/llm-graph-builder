"""
ReAct Agent V2 - Gateway Virtual Server Entegrasyonu

Bu modül, react_agent.py'nin Gateway-centric versiyonudur.
Domain yönetimi kaldırılmış, promptlar/toollar/resourcelar
IBM MCP Context Forge Gateway Virtual Server'dan dinamik olarak alınır.

Multi-Provider Support: OpenAI (GPT-5, GPT-4o) ve Anthropic (Claude Opus 4.5, Sonnet)
Prompt Caching optimizasyonlu ReAct agent sağlar.
...
# Gateway Client (domain registry yerine)
...
# ============================================================================
# MCP CONFIGURATION - V2: Gateway Virtual Server'dan alinir
# ============================================================================

# Pagination - LLM'e gosterilecek kayit sayisi (prompt'larda da kullanilir)
...
# ============================================================================
# PROMPT: Gateway'den alinir, domain-specific prompt modulleri KALDIRILDI
# ============================================================================
...
class ReactAgentV2:
    """
    Gateway-Centric ReAct Agent V2

    Domain yonetimi KALDIRILDI. Tum prompt/tool/resource bilgileri
    Gateway Virtual Server'dan (server_id) dinamik olarak alinir.

    Ozellikler:
    - OpenAI (GPT-5, GPT-4o, o1, o3) ve Anthropic (Claude Opus 4.5, Sonnet) destegi
    - Prompt Caching optimizasyonu (hem OpenAI hem Anthropic)
    - Extended Thinking destegi (GPT-5: reasoning_effort, Claude: thinking budget)
    - Paralel tool cagrilari
    - Streaming response
    - Gateway Virtual Server'dan dinamik konfigürasyon
    """
    ...
    def __init__(
        ...
        Args:
            ...
            server_id: Gateway Virtual Server ID. ZORUNLU.
                Gateway'den prompt, tool, resource bilgileri bu server_id ile alinir.
            model_name: Model adi.
            reasoning_effort: GPT-5 icin reasoning effort (none, low, medium, high)

        Raises:
            ValueError: server_id parametresi belirtilmemisse
        """
        if not server_id:
            raise ValueError("server_id parametresi zorunlu.")
        ...
        _log(f"🔧 ReactAgentV2 initialized: server_id={server_id}, model={self.model_name}")
        ...
        if REDIS_CACHE_IMPORTED and REDIS_CACHE_ENABLED and setup_semantic_cache:
            try:
                ...
                if cache_ok:
                    logging.info("✅ Redis LLM Semantic Cache aktif (ReactAgentV2)")
            except Exception as e:
                logging.warning(f"⚠️ Redis cache setup failed: {e}")

    async def _get_schema_for_session(self, session_id: str) -> str:
        """
        Session icin sema bilgisini al (cache'li).

        Oncelik:
        1. Gateway Virtual Server resources ("schema" adli resource)
        2. Neo4j direct query (fallback)
        """
        if session_id in self._schema_cache:
            return self._schema_cache[session_id]

        # 1. Gateway'den schema resource dene
        try:
            gw = await get_gateway_client()
            resources = await gw.get_server_resources(self.server_id)
            for r in resources:
                name = r.get("name", "").lower()
                if "schema" in name:
                    schema_text = r.get("text", "") or r.get("content", "")
                    if schema_text:
                        _log(f"📋 Schema from Gateway resource: {r.get('name')} ({len(schema_text)} chars)")
                        self._schema_cache[session_id] = schema_text
                        return schema_text
        except Exception as e:
    ...
    async def _create_agent(self, schema_info: str, session_id: str):
        """ReAct agent olustur - Gateway MCP endpoint uzerinden"""
        if not LANGCHAIN_AVAILABLE or create_agent is None:
            raise ImportError("LangChain not available")

        # MCP client baslat - Gateway Virtual Server MCP endpoint'i kullan
        if self.mcp_client is None:
            ...
            _log(f"✅ MCP connected via Gateway, {tool_count} tools: {tool_names}")

        # System prompt olustur (Gateway'den)
        system_prompt = await self._build_system_prompt(schema_info, session_id)
...
_react_session_agents: Dict[str, ReactAgentV2] = {}
...
async def get_or_create_react_session_agent_v2(
    session_id: str,
    server_id: str,
    ...
    """
    Session bazli ReactAgentV2 al veya olustur.
    ...
    Returns:
        ReactAgentV2 instance
    """
    global _react_session_agents, _react_session_access_times

    _cleanup_old_react_sessions()

    cache_key = f"{session_id}:{server_id}"

    if cache_key in _react_session_agents:
        _react_session_access_times[cache_key] = datetime.now()
        _log(f"ReactV2 session {session_id[:8]} (server={server_id}): reusing cached agent")
    ...
    _log(
        f"ReactV2 session {session_id[:8]} (server={server_id}): new agent created (cache={len(_react_session_agents)})"
    )
    return agent
...
async def stream_react_agent_v2_response(
    question: str,
    server_id: str,
    model: Optional[str] = None,
    session_id: str = "",
    ...
    **kwargs: Any,
    ...
    """
    ReactAgentV2 ile streaming cevap uret.

    Domain yerine server_id (Gateway Virtual Server) kullanir.
    ...
    Args:
        ...
        question_id: Soru ID'si
        graph: Neo4j graph connection
        reasoning_effort: GPT-5 icin reasoning seviyesi
        user_id: Kullanici ID (Langfuse User Tracking icin)
        **kwargs: Ek parametreler

    Yields:
        Dict: Streaming chunk'lari
    """
    if not LANGCHAIN_AVAILABLE:
        yield {
            "type": "error",
            "message": "LangChain kurulu degil.",
    ...
    if not server_id:
        yield {
            "type": "error",
            "message": "server_id parametresi zorunlu (Gateway Virtual Server ID).",
            "status": "missing_server_id",
            "timestamp": datetime.now().isoformat(),
        }
        return

    try:
        agent = await get_or_create_react_session_agent_v2(
            session_id, server_id, model, graph, reasoning_effort
        )
        ...
        logging.error(f"ReactAgentV2 streaming failed: {e}", exc_info=True)
        yield {
            "type": "error",
            "message": f"ReactAgentV2 hatasi: {str(e)}",
            "status": "failed",
            "session_id": session_id,
            "timestamp": datetime.now().isoformat(),
        }
...
def create_react_agent_v2(graph, server_id: str, **kwargs) -> ReactAgentV2:
    """
    ReactAgentV2 factory function.

    Args:
        graph: Neo4j graph connection
        server_id: Gateway Virtual Server ID
        **kwargs: ReactAgentV2 constructor arguments

    Returns:
        ReactAgentV2 instance
    """
    return ReactAgentV2(graph, server_id=server_id, **kwargs)
