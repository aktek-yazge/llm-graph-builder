        ...
        with _pool_lock:
            if _pg_pool is None:
                ...
                try:
                    ...
                    logger.info(
                        f"✅ PostgreSQL chat history pool oluşturuldu: {parsed.hostname}:{parsed.port}"
                    )
...
class PostgresSessionChatHistory:
    ...
    def get_or_create_session(
        cls, session_id: str, window: int = 50
    ...
    def clear_all_sessions(cls) -> None:
        ...
        with cls._lock:
            logger.info(
                f"🗑️ Tüm session cache temizleniyor: {len(cls._sessions)} session"
            )
            cls._sessions.clear()
...
def create_postgres_chat_message_history(
    session_id: str, write_access: bool = True, window: int = 50
) -> PostgresChatMessageHistory:
    """
