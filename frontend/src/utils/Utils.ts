...
// Regex constants to avoid Vite import analysis issues
const S3_URL_REGEX = new RegExp('^s3://([^/]+)/?$');
...
export const wikiValidation = (url: string) => {
  return url.trim() != '' && WIKI_URL_REGEX.test(url) != false;
};
export const webLinkValidation = (url: string) => {
  return (
    url.trim() != '' &&
    WEB_URL_REGEX.test(url) != false
  );
};
export const youtubeLinkValidation = (url: string) => {
  return (
    url.trim() != '' &&
    YOUTUBE_URL_REGEX.test(url) != false
  );
};
...
export const importerValidation = (url: string) => {
  return url.trim() !== '' && IMPORTER_URL_REGEX.test(url);
};
